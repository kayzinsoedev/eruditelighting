...
class ModelCatalogGallimage extends Model {
	...
	public function editGallimage($gallimage_id, $data) {
		...
		if (isset($data['gallimage_image'])) {
			foreach ($data['gallimage_image'] as $gallimage_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "gallimage_image SET gallimage_id = '" . (int)$gallimage_id . "', link = '" .  $this->db->escape($gallimage_image['link']) . "', description = '" .  $this->db->escape($gallimage_image['description']) . "', image = '" .  $this->db->escape($gallimage_image['image']) . "', sort_order = '" . (int)$gallimage_image['sort_order'] . "'");
	...
	public function getGallimageImages($gallimage_id) {
		...
		foreach ($gallimage_image_query->rows as $gallimage_image) {
			...
			$gallimage_image_data[] = array(
				...
				'description'              => $gallimage_image['description'],
