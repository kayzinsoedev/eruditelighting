...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['is_out_stock'] = false;
			if ($product_info['quantity'] <= 0) {
				$data['stock'] = $product_info['stock_status'];
				$data['is_out_stock'] = true;
			} elseif ($this->config->get('config_stock_display')) {
				$data['stock'] = $product_info['quantity'];
				$data['is_out_stock'] = false;
			} 
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
                ...
                if($option['maximum'] == "" || $option['maximum'] == 0){
                    $max = 99;
                }else{
                    $max = $option['maximum'];
                }
				$data['options'][] = array(
					...
					'char_limit'           => $this->config->get('config_length_message'),
            ...
            $data['order_link'] = $this->url->link('product/category','path=6_11&product_id=' . $this->request->get['product_id']);
		...
		if(isset($this->request->post['option']) && $this->request->post['option']) {
			foreach($this->request->post['option'] as $product_option_id => $value) {
				...
				if($result) {				
					...
					}elseif ($result['type'] == 'text' || $result['type'] == 'number'){
                        $option_value = $this->model_catalog_product->getProductOption($result['product_option_id']);
                        if($option_value['price_type'] == 2){
                            $total_str = strlen($value) - substr_count($value, ' ');
                            $option_price = $option_price + ($total_str * $option_value['price']);  
                        }else if($option_value['price_type'] == 3){
                            $total_str = strlen($value) - substr_count($value, ' ');
                            if($total_str > 1){
