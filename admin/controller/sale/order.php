...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
        ...
        $data['export'] = $this->url->link('sale/order/export', 'token=' . $this->session->data['token'], true);
	...
	public function info() {
		...
		if ($order_info) { 
			...
			$products = $this->model_sale_order->getOrderProductsSku($this->request->get['order_id']);
			...
			foreach ($products as $product) {
                ...
                $options = $this->model_sale_order->getExportOrderOptions($product['order_id'],$product['order_product_id']);
				$option_string = $product['name'];
                $sku = $product['psku'];
                $desc = 1;
                ...
                if($product['type'] == 0){
                    foreach($options as $option){
                        ...
                        if($option['product_option_value_id'] > 0){
                            $option['sku'] = $this->model_sale_order->getOptionSKU($option['product_option_value_id']);
                        }
                        if($option['size'] == 1){
                            $option_string .= " - ".$option['selection']." X ".$product['oquantity'];
                            if(isset($option['sku'])){
                                $sku .= "-".$option['sku'];
                            }
                        }else{
                            //if($desc == 1){
                                //$option_string .= "- ".strip_tags(html_entity_decode($product['description']))." ".html_entity_decode("<br>");
                                //$desc++;
                            //}
                            $option_string .= html_entity_decode("<br>").$option['name'].": ".nl2br($option['selection']);
                            
                            //$option_string .= html_entity_decode("<br>").$option['selection'];
                            $sku .= "-".$option['sku'];
                        }
                    }
                    ...
                    foreach($options as $option){
                        ...
                        if($option['size'] == 1){
                            $option_string .= " - ".$option['selection'];
                            if(isset($option['sku'])){
                                $sku .= "-".$option['sku'];
                            }
				...
				$data['products'][] = array(
					...
					'price'    		   => $this->currency->format($product['opprice'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
	...
	public function removeReward() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if (isset($this->request->get['reward_id'])) {
				$reward_id = $this->request->get['reward_id'];
			} else {
				$reward_id = 0;
			}
			...
			//if ($order_info) {
				...
				$this->model_customer_customer->deleteReward($reward_id);
			//}
	...
	public function pickPackList() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$products = $this->model_sale_order->getExcelOrderProducts($order_id);
				...
				foreach ($products as $product) {
                    ...
                    if(count($options) < 1){
                        $option_string .= " X ".$product['oquantity'];
                    }
	...
	public function export() {
        ...
        $excel->writeLine($data_array, array('text-align'=>'center', 'color'=> 'black', 'font-weight' => 'bold','border'=>'solid 1px black','background-color'=>"#b7b7b7"));
        $this->load->model('sale/order');

        foreach($orders as $order){
    		$products = $this->model_sale_order->getExportOrderProducts($order);
            ...
            foreach($products as $product){
                ...
                //if($desc == 1){
                //    $option_string .= "- ".strip_tags(html_entity_decode($product['description']."<br>"))." ";
                //    $desc++;
                //}
                ...
                $data = array(
                    ...
                    $product['firstname']." ".$product['lastname'].html_entity_decode("<br>").$product['shipping_address_1']." ".$product['shipping_address_2'].html_entity_decode("<br>")."Contact No: ".$product['telephone'],
                    $option_string,
                    $sku,
                    $product['model'],
                    $product['oquantity']
                );
                $excel->writeLine($data, array('text-align'=>'left', 'color'=> 'black'));
            }
        }
