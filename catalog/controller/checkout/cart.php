...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			//$data['checkout'] = $this->url->link('information/information','information_id=5&checkout=1', true);
		...
		if ($product_info) {
			...
			if (!$json) {
        		...
        		if( isset($this->request->post['path']) && !is_array($this->request->post['path']) ){
        			$after_clean = '&';
        			$before_clean = $this->request->post['path'];
				...
				$json['redirect'] = str_replace('&amp;', '&', $this->url->link('product/product', $after_clean.'&product_id=' . $this->request->post['product_id']));
			}
		}
