	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				$json['total_quantity'] = $this->enquiry->countProducts();
				$json['total'] = sprintf($this->language->get('text_items'), $this->enquiry->countProducts(), $this->currency->format($total, $this->session->data['currency']));
			} else {
