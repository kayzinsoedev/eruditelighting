<?php
	class ControllerInformationContact extends Controller {
		...
		public function index() {
				...
				if($this->config->get('pro_email_template_status')){
					...
					$email_params = array(
						...
						'data' => array(
							...
							//'enquiry_pax' => html_entity_decode($this->request->post['pax'], ENT_QUOTES, 'UTF-8'),
							//'enquiry_date' => $this->request->post['date'],
			...
			$data['direction'] = $this->config->get('config_direction');
...
//			}
			...
			//if (utf8_strlen($this->request->post['date']) < 3) {
			//	$this->error['date'] = $this->language->get('error_date');
			//}
			
			//if ($this->request->post['pax'] == "") {
