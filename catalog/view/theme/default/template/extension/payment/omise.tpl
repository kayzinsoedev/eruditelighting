...
<script type="text/javascript">
    $.getScript("https://cdn.omise.co/omise.min.js.gz", function() {
        ...
        $("#omise-form-checkout").submit(function() {
            ...
            <?php if ($omise_token) { ?>
                <?php /* already have card, use card to create charge */ ?>
                
                $("#input-omise-token").val('<?=$omise_token?>');
                
                if($('#omise-exist-card').is(':visible'))
                {
                    ...
                    var posting = $.post("<?php echo $checkout_url; ?>", {
                        ...
                        "selected_omise_token" : $("select#cc-stored option:checked" ).val(),
                    });
                    ...
                    if (response.object == "error") {
                        ...
                        <?php /* create customer */ ?>
                             if ($('.checkbox_save_card').is(":checked")) {
                                 $.ajax({
                                     type: 'post',
                                     url: 'index.php?route=extension/payment/omise/create_update_customer',
                                     data: $('#omise-form-checkout input[type="text"],#omise-form-checkout input[type="hidden"]'),
                                     cache: false,
                                     success: function(json) {
                                         // if already have card tagged to email address  
                                         
                                         var posting = $.post("<?php echo $checkout_url; ?>", {
                                            "omise_token": response.id,
                                            "description": "Charge a card from OpenCart that order id is <?php echo $orderid; ?> from <?php echo $billemail; ?>",
                                            "omise_customer_id": json['omise_customer_id'],
                                        });
            ...
            <?php } else { ?>
                ...
                <?php /* do not have token / customer :: create customer + create charge */ ?>
...
<script>
    function addOmiseCard() {
        $('#omise-add-card').toggle('slow');
        $('#omise-exist-card').toggle('slow');
    }
</script>
    ...
    <div class="omise-payment">
        <div class="col-lg-6 col-lg-offset-3 col-md-8 col-md-offset-2 col-sm-10 col-sm-offset-1 col-xs-12">
            ...
            <?php if ( $omise_token )  { ?>
                ...
                <div id="omise-exist-card">
                    <div class="form-group">
                        <label class="bold" for="cc-stored">Pay with </label>
                        <select class="form-control" id="cc-stored">
                            <?php foreach($omise_card as $omise_c){ ?>
                                <option value="<?=$omise_c['omise_token']?>"><?=$omise_c['omise_card_no']?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                <hr>
                <a href="javascript:addOmiseCard()" class="text-use-card">Use another card</a><br>
                <hr>
                <div id="omise-add-card" style="display:none">
                    <!-- Card Holder Name -->
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label class="bold"><input type="checkbox" class="checkbox_save_card"> Save this card</label>
                    </div>
