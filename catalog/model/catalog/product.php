	...
	class ModelCatalogProduct extends Model {
		...
		public function getProduct($product_id) {
			...
			if ($query->num_rows) {
				...
				'date_special_start' => $query->row['date_special_start'],
				'date_special_end' => $query->row['date_special_end'],
		...
		public function getProducts($data = array(), $sub_query = false) {
			...
			$today = date('Y-m-d');
			$sql .= " AND ((date_special_start = '0000-00-00') OR (date_special_start <= '$today' AND date_special_end >= '$today'))";
		...
		public function getDiscountAmountForUpdatePrice($product_id, $quantity) {					
			...
			$product_option = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option WHERE product_option_id = '" . (int)$product_option_id . "'");

                        return $product_option->row;
		}
