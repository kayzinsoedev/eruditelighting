		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
					$option_query = $this->db->query("SELECT po.product_option_id, po.option_id, od.name, o.type, po.price_type, po.price FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.product_option_id = '" . (int)$product_option_id . "' AND po.product_id = '" . (int)$cart['product_id'] . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'");
					...
					if ($option_query->num_rows) {
						...
						} elseif ($option_query->row['type'] == 'text' || $option_query->row['type'] == 'number') {
                            $option = json_decode($cart['option'],true);
                            if (array_key_exists($option_query->row['product_option_id'],$option)){
                                if($option_query->row['price_type'] == 2){
                                    $len = strlen($option[$option_query->row['product_option_id']]);
                                    $option_price += ($option_query->row['price'] * $len); 
                                }else if($option_query->row['price_type'] == 3){
                                    $len = strlen($option[$option_query->row['product_option_id']]);
                                    if($len > 1){
                                        $len = $len - 1;
                                        $option_price += ($option_query->row['price'] * $len);
				...
				$product_data[] = array(
					...
					'date_special_start'  => $product_query->row['date_special_start'],
					'date_special_end'  => $product_query->row['date_special_end'],
					'sku'			  => $sku,
