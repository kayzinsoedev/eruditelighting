<?php
//==============================================================================
// Category & Product-Based Shipping v302.4
// 
...
class ModelExtensionShippingCategoryProductBased extends Model {
	...
	public function getQuote($address) {
		...
		foreach ($settings['charge'] as $charge) {
			...
			if (strpos($charge['type'], 'text_') === 0) {
			    ...
			    $discount = $this->checkDiscount($total);
			    $reward = $this->getRewardTotal($total);
			    $credit = $this->getCreditTotal($total);
				$percentage_total = $total - $discount - $reward - $credit;
				
				$cost = $this->calculateBrackets($brackets, $charge['type'], $percentage_total, $quantity, $percentage_total);
				if ($cost !== false) {
					$rate_found = true;
				}
		...
		foreach ($settings['combination'] as $combination) {
			...
			$quote_data[$this->name . '_' . count($quote_data)] = array(
				...
				'title'			=> ($combination['title'] == 'single') ? $titles[0] : implode(' + ', array_filter($titles)),
				'cost'			=> $cost,
				'value'			=> $cost,
	...
	private function checkDiscount($total,$order_id=0) {
	    $discount_total = 0;
	...
	private function getCoupon($code,$order_id=0) {
		$status = true;
	...
	private function getRewardTotal($total) {
	    $points_to_cost = 0;
        ...
        return $points_to_cost;
	}
	
	private function getRewardInfoByCustomerGroup($customer_group_id=0){
	...
	private function getCreditTotal($total) {
	    $credit = 0;
