          ...
          <div class="tab-content">
                  ...
                  <tbody id="cart">
                    ...
                    <tr>
                      <td class="text-left"><?php echo $order_product['name']; ?><br />
                        ...
                        - <small><?php echo $option['name']; ?>: <?php echo nl2br($option['value']); ?></small><br />
                        ...
                        <input type="hidden" name="product[<?php echo $product_row; ?>][option][<?php echo $option['product_option_id']; ?>][]" value="<?php echo $option['product_option_value_id']; ?>" />
                        <?php } ?>
                        <?php if ($option['type'] == 'text' || $option['type'] == 'textarea' || $option['type'] == 'file' || $option['type'] == 'number' || $option['type'] == 'date' || $option['type'] == 'datetime' || $option['type'] == 'time') { ?>
            ...
            <div class="tab-pane" id="tab-total">
              ...
              <fieldset>
                ...
                <div class="form-group">
                  <label class="col-sm-2 control-label" for="input-delivery_date"><?php echo "Delivery Date"; ?></label>
                  <div class="col-sm-10 date">
                      <input type="text" name="delivery_date" value="<?php echo date("m/d/Y",strtotime($delivery_date)); ?>" id="input-delivery_date" class="form-control" />
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-2 control-label" for="input-delivery_time"><?php echo "Delivery Time"; ?></label>
                  <div class="col-sm-10">
                      <input type="text" name="delivery_time" value="<?php echo $delivery_time; ?>" id="input-delivery_time" class="form-control" />
                  </div>
                </div>
                <div class="form-group">
...
$('#button-refresh').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/cart/products&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val() + '&order_id='+$('input[name=\'order_id\']').val(),
		dataType: 'json',
		crossDomain: true,
		success: function(json) {
				...
				for (i = 0; i < json['products'].length; i++) {
					...
					if (product['option']) {
						for (j = 0; j < product['option'].length; j++) {
							...
							html += '  - <small>' + option['name'] + ': ' + nl2br(option['value']) + '</small><br />';
...
$('#button-coupon').on('click', function() {
	$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/coupon&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val() + '&order_id='+$('input[name=\'order_id\']').val(),
		type: 'post',
...
$('#button-save').on('click', function() {
	...
	$.ajax({
		url: url,
		type: 'post',
		data: $('select[name=\'payment_method\'] option:selected,  select[name=\'shipping_method\'] option:selected,  #tab-total select[name=\'order_status_id\'], #tab-total select, #tab-total textarea[name=\'comment\'], #tab-total input[name=\'affiliate_id\'], input[name=\'delivery_date\'], input[name=\'delivery_time\']'),
		dataType: 'json',
		crossDomain: true,
		beforeSend: function() {
