...
class ModelCheckoutOrder extends Model {
	...
	public function getOrderProductIds($order_id) {
        ...
        public function getTotalOrderSlot($shipping_method, $delivery_date, $delivery_time) {
		$sql = "SELECT count(*) AS total FROM " . DB_PREFIX . "order " .
		"WHERE shipping_method = '" . $shipping_method . "' AND delivery_date = '" . $delivery_date . "' AND delivery_time = '" . $delivery_time . "'";
		return $this->db->query($sql)->row['total'];
	}

	public function addOrder($data) {
	    if($data['shipping_code'] == "category_product_based.category_product_based_0"){
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				foreach ($product['option'] as $option) {
				    $sql = "SELECT option_id FROM " . DB_PREFIX . "product_option WHERE product_option_id = '" . (int)$option['product_option_id'] . "'";
				    $query = $this->db->query($sql);
				    
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', option_id = '" . $query->row['option_id'] . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', name = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "'");
				}
			}
		}
			...
			foreach ($data['totals'] as $total) {
                if($total['code'] == "shipping" || $total['code'] == "total"){
                    if($data['delivery_date'] != "" && $data['delivery_time'] != ""){
                        ...
                        if($data['shipping_method'] == "Self Pickup"){
                        ...
                        if(gettype($default_setting) == "array"){
                            foreach($default_setting as $setting){
                                foreach($setting as $key1 => $value1){
                                    if($key1 == "delivery_times" && $value1 == $data['delivery_time']){
                        ...
                        if(gettype($special_setting) == "array"){
                            foreach($special_setting as $arr => $setting){
                                if($setting['product_group'] == ""){
                                    if($setting["type"] == 2 && $setting['add_time'] == $data['delivery_time'] && $setting['delivery_date'] == $data['delivery_date']){
